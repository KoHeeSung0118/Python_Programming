...
print(id(a[0]), id(a[0]))  # 이미 만든게 있다면 재사용(새로 ID부여하지 않음)
...
# 슬라이싱 (start:end-1:step)  개수로써 생각하면 편함(end에서 start뺀 것이 개수)
print(a[0:2])  # Py 출력
...
s = "1401"
print(f"{s[0]}학년 {s[1]}반 {int(s[2] + s[3])}번")  # ✅ "1학년 4반 1번" 출력

# # 2️⃣ 학번을 슬라이싱만을 이용해서 학년, 반, 번호로 잘라내기
print(f"{s[0:1:]}학년 {s[1:2]}반 {int(s[2:4])}번")  # ✅ "1학년 4반 1번" 출력
